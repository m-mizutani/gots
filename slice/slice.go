...
// FromMap creates map[K]V from slice and func(T) (K, V)
func FromMap[K comparable, V, T any](input map[K]V, f func(K, V) T) []T {
	var resp []T
...
func FromValues[K comparable, V any](input map[K]V) []V {
	resp := make([]V, len(input))
	i := 0
	for k := range input {
		resp[i] = input[k]
		i++
...
func FromKeys[K comparable, V any](input map[K]V) []K {
	resp := make([]K, len(input))
	i := 0
	for k := range input {
		resp[i] = k
		i++
